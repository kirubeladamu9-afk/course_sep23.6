...
const CourseEditorSection: FC<{ title: string; description?: string; expanded: boolean; onToggle: () => void; withTopMargin?: boolean; children: ReactNode }> = ({ title, description, expanded, onToggle, withTopMargin = false, children }) => (
  <Accordion expanded={expanded} onChange={onToggle} disableGutters elevation={0} sx={{ width: '100%', mt: withTopMargin ? 2 : 0, border: 1, borderColor: 'divider', borderRadius: 1, overflow: 'hidden', '&:before': { display: 'none' } }}>
    <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ minHeight: 64, '&.Mui-expanded': { minHeight: 64 }, '& .MuiAccordionSummary-content, & .MuiAccordionSummary-content.Mui-expanded': { my: 2 } }}><Box><Typography variant="h6">{title}</Typography>{description && <Typography color="text.secondary" variant="body2">{description}</Typography>}</Box></AccordionSummary>
    <AccordionDetails sx={{ p: { xs: 2, md: 2.5 } }}>{children}</AccordionDetails>
  </Accordion>
)
...
const createLesson = (id: number, type: LessonType): AdminLesson => ({
  ...
  ...(type === 'stem_lab' ? { subtype: undefined, config: undefined, stemLabPublished: false } : {}),
...
export const CourseEditor: FC<{ course: AdminCourse; onChange: (course: AdminCourse) => void; isClassLinked?: boolean; curriculumOnly?: boolean; allowedLessonTypes?: LessonType[]; classSchedule?: ClassLessonSchedule; classMeetingLink?: string }> = ({ course, onChange, isClassLinked = false, curriculumOnly = false, allowedLessonTypes, classSchedule, classMeetingLink = '' }) => {
  ...
  const openLessonPanel = (moduleId: number, lesson: AdminLesson, isNew: boolean) => {
    setAddingLessonModuleId(null)
    setVideoUploadProgress(0)
    const lessonCopy = { ...lesson, resources: [...lesson.resources], interactiveHotspots: lesson.interactiveHotspots?.map((hotspot) => ({ ...hotspot })), quizQuestions: lesson.quizQuestions?.map((question) => ({ ...question, options: [...question.options] })), practiceQuestions: lesson.practiceQuestions?.map((question) => ({ ...question, options: [...question.options] })) }
...
const LessonDetailsPanel: FC<{ lessonPanel: LessonPanelState; updateLessonDraft: (lesson: AdminLesson) => void; onClose: () => void; onSave: () => void; onVideoFile: (file: File) => void; onAddResources: (files: FileList | null) => void; videoUploadProgress: number; disabled: boolean; classSchedule?: ClassLessonSchedule; classMeetingLink?: string; availableLiveSessionDates: string[] }> = ({ lessonPanel, updateLessonDraft, onClose, onSave, onVideoFile, onAddResources, videoUploadProgress, disabled, classSchedule, classMeetingLink = '', availableLiveSessionDates }) => {
  ...
  return <Paper elevation={0} sx={{ mt: 2, p: 2.5, border: 1, borderColor: 'primary.main', backgroundColor: 'background.paper' }}>
    ...
    <Stack spacing={2}><TextField label="Title" fullWidth required value={lesson.title} onChange={(event) => updateLessonDraft({ ...lesson, title: event.target.value })} />
      ...
      {lesson.type === 'stem_lab' && <StemLabActivityEditor lesson={lesson} updateLessonDraft={updateLessonDraft} />}
...
const StemLabsPage: FC = () => {
  ...
  const loadLabs = async () => {
    ...
    try {
      const courses = await getAdminCourses()
      setLabs(courses.flatMap((course) => course.modules.flatMap((module) => module.lessons.filter((lesson) => lesson.type === 'stem_lab').map((lesson) => {
        const subject = lesson.subtype?.split('.')[0]
        return {
  ...
  const labsBySubject = useMemo(() => STEM_SUBJECTS.reduce<Record<StemSubject, StemLabSummary[]>>((groups, subject) => {
    groups[subject] = labs.filter((lab) => lab.subject === subject)
    return groups
  }, { math: [], physics: [], chemistry: [], biology: [] }), [labs])
  const publishedCount = labs.filter((lab) => lab.published).length
    ...
    {isLoading ? <AdminLoadingState label="Loading STEM Labs" /> : loadError ? <Paper elevation={0} sx={{ p: 4, border: 1, borderColor: 'divider' }}><Typography color="error" sx={{ mb: 2 }}>{loadError}</Typography><Button label="Retry" onClick={() => void loadLabs()} /></Paper> : <>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'repeat(2, minmax(0, 1fr))' }, gap: 2 }}>
        {STEM_SUBJECTS.map((subject) => {
          ...
          return <Paper key={subject} elevation={0} sx={{ p: { xs: 2, md: 2.5 }, border: 1, borderColor: 'divider' }}>
