...
import { beginQuizAttempt, completeLesson as completeLessonApi, getAuthenticatedUser, getBookstorePurchases, getCourses, getGamification, getMyEnrollments, getMyPayments, getPracticeExam, getPracticePurchases, getPublicClasses, logLiveSessionJoin, recordPracticeLessonAnswer, recordStemAttempt, saveAuthenticatedUser, saveLessonEngagement, saveQuizAnswer, saveQuizViolation, submitQuizAttempt, signOut, updateAuthenticatedProfile, type BookstorePurchase, type GamificationData, type MyEnrollment, type MyPayment, type PublicClass, type QuizAnswerRecord, type QuizAnswerStatus, type QuizAttempt, type QuizQuestionResult, type QuizViolation, type QuizViolationType, type WeakArea } from '@/services/api'
import { navigateTo } from '@/lib/navigation'
import { calculateOverallGrade } from '@/lib/overall-grade'
import { StemActivityPlayer } from '@/components/stem/stem-lab'
import { STEM_GRADE_BAND_LABELS, STEM_SUBJECT_LABELS, type StemActivityAttempt, type StemActivityResult, type StemLabConfig, type StemSubtype } from '@/components/stem/stem-types'
...
const WeakAreasPanel: FC<{ enrollments: Array<Pick<DashboardEnrollment, 'id' | 'item_id' | 'weakAreas'>>; onOpenCourse: (courseId: number, lessonId?: number) => void }> = ({ enrollments, onOpenCourse }) => {
  ...
  return <Paper elevation={0} sx={{ p: 2.5, mb: 3, border: 1, borderColor: areas.length ? 'warning.light' : 'divider', backgroundColor: areas.length ? (theme) => alpha(theme.palette.warning.main, theme.palette.mode === 'dark' ? 0.16 : 0.08) : 'background.paper' }}><Typography component="h2" variant="h6" sx={{ mb: 0.5 }}>Weak Areas</Typography><Typography color="text.secondary" variant="body2" sx={{ mb: areas.length ? 2 : 0 }}>{areas.length ? 'These topics are below 60% accuracy. Practice them again to build confidence.' : 'No weak areas detected yet. Complete quizzes, practice exercises, or STEM Lab activities to see topic-level results here.'}</Typography>{areas.length > 0 && <Stack spacing={1}>{areas.map(({ enrollment, area }) => <Stack key={`${enrollment.id}-${area.subject ?? ''}-${area.gradeBand ?? ''}-${area.topic}`} direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={1} sx={{ p: 1.25, borderRadius: 1.5, backgroundColor: 'background.paper' }}><Box><Stack direction="row" spacing={1} alignItems="center"><Typography sx={{ fontWeight: 700 }}>{area.topic}</Typography>{area.subject && <Chip size="small" variant="outlined" label={STEM_SUBJECT_LABELS[area.subject as keyof typeof STEM_SUBJECT_LABELS] ?? area.subject} />}{area.gradeBand && <Chip size="small" variant="outlined" label={STEM_GRADE_BAND_LABELS[area.gradeBand as keyof typeof STEM_GRADE_BAND_LABELS] ?? area.gradeBand} />}<Chip size="small" color="warning" label={`${area.accuracy}% accuracy`} /></Stack><Typography variant="body2" color="text.secondary">{area.correct} of {area.total} answers correct · Practice this topic again.</Typography></Box>{area.practiceLessons[0] && <Button size="small" variant="outlined" onClick={() => onOpenCourse(enrollment.item_id, area.practiceLessons[0].lessonId)}>Practice {area.practiceLessons[0].lessonTitle}</Button>}</Stack>)}</Stack>}</Paper>
}
...
const CourseViewer: FC<{ course: DashboardCourse; progress: number; completedLessonIds: number[]; started: boolean; timeSpentSeconds: number; quizResults: Record<number, DashboardQuizResult>; weakAreas: WeakArea[]; attendance: Record<number, 'Present' | 'Absent'>; isClass: boolean; now: Date; initialLessonId?: number; onBack: () => void; onStart: () => void; onCompleteLesson: (lessonId: number, stemResult?: StemActivityResult) => Promise<void>; onStemAttempt: (lessonId: number, attempt: StemActivityAttempt) => void; onQuizStart: (lessonId: number) => Promise<QuizAttempt | null>; onQuizAnswer: (lessonId: number, attemptId: number, questionId: number, answer: QuizAnswerRecord) => Promise<void>; onQuizSubmit: (lessonId: number, attemptId: number, answers: Record<number, QuizAnswerRecord>, disqualified?: boolean) => Promise<DashboardQuizResult | null>; onQuizViolation: (lessonId: number, attemptId: number, violation: QuizViolation) => Promise<void>; onQuizActiveChange: (active: boolean) => void; onPracticeAnswer: (lessonId: number, questionId: number, answer: string) => void; sessionJoinClicks: Record<number, string>; onJoinLiveSession: (lessonId: number, meetingUrl: string) => Promise<void>; readOnly?: boolean }> = ({ course, progress, completedLessonIds, started, timeSpentSeconds, quizResults, weakAreas, attendance = {}, isClass, now, initialLessonId, onBack, onStart, onCompleteLesson, onStemAttempt, onQuizStart, onQuizAnswer, onQuizSubmit, onQuizViolation, onQuizActiveChange, onPracticeAnswer, sessionJoinClicks, onJoinLiveSession, readOnly = false }) => {
  ...
  if (isStemLabLesson(selectedLesson)) return <>
    <Box component="button" type="button" onClick={onBack} sx={{ display: 'flex', alignItems: 'center', gap: 0.5, p: 0, mb: 3, border: 0, background: 'none', color: 'primary.main', cursor: 'pointer', font: 'inherit' }}><ArrowBackIcon fontSize="small" /> Back to Course</Box>
    <StemActivityPlayer lesson={selectedLesson} onAttempt={(attempt) => onStemAttempt(selectedLesson.id, attempt)} onResult={(result) => { void onCompleteLesson(selectedLesson.id, result).then(() => setStemLabPassed(true)) }} />
...
const StudentDashboard: FC<StudentDashboardProps> = ({ darkMode, onToggleDarkMode }) => {
  ...
  const recordStemActivityAttempt = (lessonId: number, attempt: StemActivityAttempt) => {
    if (!selectedCourseEnrollment) return
    void recordStemAttempt(selectedCourseEnrollment.id, lessonId, attempt).catch((error) => setProgressError(error instanceof Error ? error.message : 'Unable to record STEM Lab feedback.'))
  }
