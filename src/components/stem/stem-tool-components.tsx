...
type ChemistryGraphPoint = { x: number; y: number }

const ChemistrySimulationGraph: FC<{ scenario: StemChemistrySimulationConfig['scenario']; values: Record<string, string | undefined> }> = ({ scenario, values }) => {
  const points: ChemistryGraphPoint[] = scenario === 'reaction_rate'
    ? Array.from({ length: 13 }, (_, index) => { const temperature = index * 5; return { x: temperature, y: Number(values.concentration) * 2 ** ((temperature - 20) / 10) } })
    : Array.from({ length: 13 }, (_, index) => { const baseVolume = index * 5; const acid = Number(values.acidMolarity) * Number(values.acidVolume); const base = Number(values.baseMolarity) * baseVolume; return { x: baseVolume, y: (acid - base) } })
  const width = 520
  const height = 230
  const padding = { top: 24, right: 20, bottom: 34, left: 48 }
  const minY = Math.min(...points.map((point) => point.y), 0)
  const maxY = Math.max(...points.map((point) => point.y), 1)
  const xFor = (value: number) => padding.left + (value / Math.max(1, points[points.length - 1].x)) * (width - padding.left - padding.right)
  const yFor = (value: number) => height - padding.bottom - ((value - minY) / Math.max(1e-9, maxY - minY)) * (height - padding.top - padding.bottom)
  const line = points.map((point) => `${xFor(point.x)},${yFor(point.y)}`).join(' ')
  const zeroY = yFor(0)
  return <Box><Typography variant="subtitle2" sx={{ mb: 0.5 }}>{scenario === 'reaction_rate' ? 'Reaction rate across temperature' : 'Acid excess across base volume'}</Typography><Box component="svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={scenario === 'reaction_rate' ? 'Line graph of reaction rate across temperature' : 'Line graph of acid excess across base volume'} sx={{ width: '100%', height: 'auto', border: 1, borderColor: 'divider', borderRadius: 1, backgroundColor: 'background.default' }}><line x1={padding.left} y1={zeroY} x2={width - padding.right} y2={zeroY} stroke="currentColor" opacity=".25" /><polyline points={line} fill="none" stroke="primary.main" strokeWidth="4" strokeLinejoin="round" strokeLinecap="round" /><text x={width / 2} y={height - 8} textAnchor="middle" fontSize="12" fill="currentColor">{scenario === 'reaction_rate' ? 'Temperature (°C)' : 'Base volume (mL)'}</text><text x="14" y={height / 2} textAnchor="middle" fontSize="12" fill="currentColor" transform={`rotate(-90 14 ${height / 2})`}>{scenario === 'reaction_rate' ? 'Relative rate' : 'Acid excess (mmol)'}</text></Box></Box>
}
...
export const ChemistrySimulationPlayer: FC<StemToolPlayerProps> = ({ config, onComplete }) => {
  ...
  const ready = Object.values(values).every((value) => value.trim() && Number.isFinite(Number(value)) && Number(value) >= 0) && (item.scenario !== 'reaction_rate' || Number(values.concentration) > 0)
  return <Stack spacing={2}><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 1 }}>{Object.entries(values).map(([key, value]) => <TextField key={key} type="number" label={key.replace(/([A-Z])/g, ' $1')} value={value} onChange={(event) => setValues({ ...values, [key]: event.target.value })} inputProps={{ min: 0, step: 'any' }} />)}</Box><Button variant="contained" disabled={!ready} onClick={run} startIcon={<PlayArrowIcon />}>Run simulation</Button>{result && <Paper variant="outlined" sx={{ p: 2 }}><Typography sx={{ fontWeight: 800 }}>Calculated state</Typography>{Object.entries(result).map(([key, value]) => <Typography key={key} variant="body2">{key.replace(/([A-Z])/g, ' $1')}: {typeof value === 'number' ? numberText(value) : value}</Typography>)}<ChemistrySimulationGraph scenario={item.scenario} values={values} /><Button sx={{ mt: 1 }} variant="outlined" onClick={() => onComplete({ scenario: item.scenario, input: values, output: result })}>Record simulation</Button></Paper>}</Stack>
}
...
export const LabEnvironmentPlayer: FC<StemToolPlayerProps> = ({ config, onComplete }) => {
  ...
  const isFixedExperiment = item.activity === 'experiment'
  ...
  const ready = equipment.every((entry) => placed.includes(entry)) && Object.values(values).every((value) => value.trim() && Number.isFinite(Number(value)) && Number(value) >= 0)
  ...
  const observationTerms = outcome ? item.observationKeywords?.length ? item.observationKeywords : item.scenario === 'neutralization' ? (isFixedExperiment ? ['neutral', 'endpoint'] : outcome.detail.toLowerCase().includes('acid') ? ['acid', 'excess'] : outcome.detail.toLowerCase().includes('base') ? ['base', 'excess'] : ['neutral', 'endpoint']) : item.scenario === 'pendulum' ? ['period'] : outcome.detail.toLowerCase().includes('isotonic') ? ['isotonic'] : outcome.detail.toLowerCase().includes('out') ? ['water', 'out'] : ['water', 'into'] : []
  const observationValid = Boolean(observation.trim() && observationTerms.every((term) => observation.toLowerCase().includes(term)))
  const canRecord = Boolean(outcome && completedSteps.length === item.steps.length && observationValid)
  return <Stack spacing={2}><Typography variant="subtitle1" sx={{ fontWeight: 800 }}>{isFixedExperiment ? 'Guided experiment workspace' : 'Exploratory virtual lab workspace'} · {labScenarioLabel[item.scenario]}</Typography><Typography variant="body2" color="text.secondary">{isFixedExperiment ? 'Follow the fixed procedure and record the expected result.' : 'Adjust the input variables, run the lab, and compare how the outcome changes.'}</Typography><Stack direction="row" spacing={0.75} useFlexGap flexWrap="wrap">{equipment.filter((entry) => !placed.includes(entry)).map((entry) => <Chip key={entry} label={`Drag ${entry}`} draggable onDragStart={() => setDragged(entry)} sx={{ cursor: 'grab' }} />)}</Stack><Paper variant="outlined" onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragged && !placed.includes(dragged)) setPlaced((current) => [...current, dragged]); setDragged(null) }} sx={{ minHeight: 130, p: 2, borderStyle: 'dashed', backgroundColor: 'background.default' }}><Typography variant="body2" color="text.secondary">Drag every required item here to set up the lab.</Typography><Stack direction="row" spacing={0.75} useFlexGap flexWrap="wrap" sx={{ mt: 1 }}>{placed.map((entry) => <Chip key={entry} color="primary" label={entry} onDelete={() => setPlaced((current) => current.filter((value) => value !== entry))} />)}</Stack></Paper><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 1 }}>{labInputs[item.scenario].map(({ key, label }) => <TextField key={key} type="number" label={label} value={values[key] ?? ''} onChange={(event) => setValues({ ...values, [key]: event.target.value })} disabled={isFixedExperiment} inputProps={{ min: 0, step: 'any' }} />)}</Box><Button variant="contained" disabled={!ready} startIcon={<PlayArrowIcon />} onClick={run}>Run {item.activity === 'experiment' ? 'experiment' : 'lab'}</Button>{outcome && <Paper variant="outlined" sx={{ p: 2 }}><Typography sx={{ fontWeight: 800 }}>{outcome.metric}: {numberText(outcome.value)} {outcome.unit}</Typography><Typography variant="body2" color="text.secondary">{outcome.detail}</Typography></Paper>}<Stack spacing={0.5}>{item.steps.map((step, index) => <Box key={`${step}-${index}`} component="label" sx={{ display: 'flex', alignItems: 'center', gap: 1, cursor: 'pointer' }}><input type="checkbox" checked={completedSteps.includes(index)} disabled={!outcome} onChange={() => markStep(index)} /> <Typography variant="body2">{step}</Typography></Box>)}</Stack><TextField fullWidth multiline minRows={2} label="Observation" value={observation} onChange={(event) => setObservation(event.target.value)} disabled={!outcome} helperText={observationTerms.length ? `Include: ${observationTerms.join(' + ')}.` : 'Record an observation based on the calculated lab outcome.'} error={Boolean(observation.trim() && !observationValid)} /><Button variant="outlined" disabled={!canRecord} onClick={() => onComplete({ scenario: item.scenario, equipment: placed, input: values, outcome, observation, completedSteps })}>Record {item.activity === 'experiment' ? 'experiment' : 'lab'} result</Button></Stack>
}
