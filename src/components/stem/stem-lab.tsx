...
import { STEM_CURRICULUM_CATEGORY_SUBJECTS, STEM_CURRICULUM_CATEGORY_VALIDATION, type StemCategorySubject, type StemCurriculumCategory } from './stem-curriculum-categories'
...
const curriculumSubjectToStemSubject: Record<StemCategorySubject, StemSubject> = {
  mathematics: 'math',
  physics: 'physics',
  chemistry: 'chemistry',
  biology: 'biology',
...
export const StemLabEditor: FC<{ lesson: AdminLesson; updateLessonDraft: (lesson: AdminLesson) => void }> = ({ lesson, updateLessonDraft }) => {
  ...
  const [curriculumSubject, setCurriculumSubject] = useState<StemCategorySubject | undefined>()
  ...
  const curriculumSubjectEntry = STEM_CURRICULUM_CATEGORY_SUBJECTS.find((entry) => entry.subject === curriculumSubject)
  ...
  const chooseCurriculumCategory = (category: StemCurriculumCategory) => {
    if (!curriculumSubject) return
    setCurriculumCategory(category)
  }
  const useCurriculumCategory = () => {
    if (!curriculumSubject || !curriculumCategory) return
    const nextSubject = curriculumSubjectToStemSubject[curriculumSubject]
    setSelectedSubject(nextSubject)
    setSelectedGradeBand(undefined)
    updateLessonDraft({ ...lesson, title: `${curriculumSubjectEntry?.label ?? curriculumSubject} · ${curriculumCategory.label}`, subtype: undefined, config: undefined, stemLabPublished: false })
    setPreviewed(false)
    setStage('tool')
  }
  ...
  return <Stack spacing={2.5}>
    ...
    {stage === 'subject' && <Stack spacing={1.5}><Typography variant="subtitle2" sx={{ fontWeight: 700 }}>Choose how to start</Typography><Paper component="button" type="button" variant="outlined" onClick={() => setStage('curriculum')} sx={{ p: 2, textAlign: 'left', color: 'text.primary', backgroundColor: 'background.default', cursor: 'pointer', borderColor: 'primary.main', '&:hover': { backgroundColor: 'action.hover' } }}><Typography sx={{ fontWeight: 800 }}>Browse by Curriculum Category</Typography><Typography variant="body2" color="text.secondary">Start with a reusable subject category and activity pattern, then continue to the existing tool picker.</Typography><Typography variant="caption" color="primary.main">{STEM_CURRICULUM_CATEGORY_VALIDATION.subjectCount} subjects · {STEM_CURRICULUM_CATEGORY_VALIDATION.categoryCount} categories · {STEM_CURRICULUM_CATEGORY_VALIDATION.coveredTopicCount} covered topic references</Typography></Paper><Typography variant="subtitle2" sx={{ fontWeight: 700, mt: 1 }}>Browse by Subject / Tool</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 1 }}>{STEM_SUBJECTS.map((item) => <Paper key={item} component="button" type="button" variant="outlined" onClick={() => chooseSubject(item)} sx={{ minHeight: 92, p: 2, textAlign: 'left', color: 'text.primary', backgroundColor: 'background.default', cursor: 'pointer', borderColor: subject === item ? 'primary.main' : 'divider', '&:hover': { borderColor: 'primary.main' } }}><Typography sx={{ fontWeight: 800 }}>{STEM_SUBJECT_LABELS[item]}</Typography><Typography variant="body2" color="text.secondary">{getStemToolsForSubject(item).length} available tools</Typography></Paper>)}</Box></Stack>}
    {stage === 'curriculum' && <StemCurriculumCategoryBrowser selectedSubject={curriculumSubject} selectedCategory={curriculumCategory} onSubjectChange={(nextSubject) => { setCurriculumSubject(nextSubject); setCurriculumCategory(undefined) }} onCategorySelect={chooseCurriculumCategory} onUseCategory={useCurriculumCategory} onBack={() => setStage('subject')} />}
