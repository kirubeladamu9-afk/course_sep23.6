...
import { type StemCurriculumCategory } from './stem-curriculum-categories'
import { StemCurriculumCategoryBrowser } from './stem-curriculum-browser'
...
export const StemLabEditor: FC<{ lesson: AdminLesson; updateLessonDraft: (lesson: AdminLesson) => void }> = ({ lesson, updateLessonDraft }) => {
  ...
  const [curriculumCategory, setCurriculumCategory] = useState<StemCurriculumCategory | undefined>()
  ...
  useEffect(() => {
    ...
    setCurriculumCategory(undefined)
  ...
  const chooseCurriculumCategory = (category: StemCurriculumCategory) => setCurriculumCategory((current) => current?.id === category.id ? undefined : category)
  const chooseTool = (nextSubject: StemSubject, nextTool: StemTool) => {
    ...
    if (curriculumCategory) nextConfig.topic = curriculumCategory.label
    const nextTitle = curriculumCategory ? `${STEM_SUBJECT_LABELS[nextSubject]} · ${curriculumCategory.label}` : lesson.title
  ...
  return <Stack spacing={2.5}>
    ...
    {stage === 'subject' && <Stack spacing={1.5}><Typography variant="subtitle2" sx={{ fontWeight: 700 }}>Choose a subject</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 1 }}>{STEM_SUBJECTS.map((item) => <Paper key={item} component="button" type="button" variant="outlined" onClick={() => chooseSubject(item)} sx={{ minHeight: 92, p: 2, textAlign: 'left', color: 'text.primary', backgroundColor: 'background.default', cursor: 'pointer', borderColor: subject === item ? 'primary.main' : 'divider', '&:hover': { borderColor: 'primary.main' } }}><Typography sx={{ fontWeight: 800 }}>{STEM_SUBJECT_LABELS[item]}</Typography><Typography variant="body2" color="text.secondary">{getStemToolsForSubject(item).length} available tools plus curriculum patterns</Typography></Paper>)}</Box></Stack>}
    {stage === 'tool' && <Stack spacing={1.5}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}><FormControl fullWidth size="small"><InputLabel>Subject</InputLabel><Select label="Subject" value={subject ?? ''} onChange={(event) => chooseSubject(event.target.value as StemSubject)}>{STEM_SUBJECTS.map((item) => <MenuItem key={item} value={item}>{STEM_SUBJECT_LABELS[item]}</MenuItem>)}</Select></FormControl><FormControl fullWidth size="small" disabled={!subject}><InputLabel>Grade</InputLabel><Select label="Grade" value={selectedGrade} onChange={(event) => chooseGrade(Number(event.target.value))}>{STEM_GRADES.map((grade) => <MenuItem key={grade} value={grade}>Grade {grade}</MenuItem>)}</Select></FormControl></Stack>{subject && <StemCurriculumCategoryBrowser subject={subject === 'math' ? 'mathematics' : subject} gradeBand={gradeBand} selectedCategory={curriculumCategory} onCategorySelect={chooseCurriculumCategory} />}{subject && <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, flexWrap: 'wrap' }}><Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{curriculumCategory ? `Activity tools for ${curriculumCategory.label}` : 'Curriculum activity tools'}</Typography><Typography variant="caption" color="text.secondary">{curriculumCategory ? 'Choose an implementation tool to open Configure. Preview unlocks after required fields are complete.' : 'Choose a category pattern first, then select its implementation tool.'}</Typography></Box>}{subject && curriculumCategory ? <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 1 }}>{getStemToolsForSubject(subject).filter((item) => !curriculumCategory || curriculumCategory.toolSubtypes.includes(item.subtype)).map((item) => <Paper key={item.subtype} component="button" type="button" variant="outlined" onClick={() => chooseTool(item.subject, item.tool)} sx={{ minHeight: 92, display: 'flex', alignItems: 'center', gap: 1.25, p: 1.5, textAlign: 'left', color: 'text.primary', backgroundColor: 'background.default', cursor: 'pointer', borderColor: definition?.subtype === item.subtype ? 'primary.main' : 'divider', '&:hover': { borderColor: 'primary.main' } }}><Box sx={{ color: 'primary.main', display: 'flex' }}>{item.icon}</Box><Box sx={{ minWidth: 0 }}><Typography sx={{ fontWeight: 800 }}>{item.label}</Typography><Chip size="small" label={item.mode === 'build' ? 'Interactive build' : 'Embedded tool'} variant="outlined" /></Box></Paper>)}</Box> : <Typography color="text.secondary">Choose a subject and grade band first.</Typography>}</Stack>}
