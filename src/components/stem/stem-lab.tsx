...
import { STEM_SUBJECT_LABELS, STEM_SUBJECTS, type StemActivityAttempt, type StemActivityResult, type StemGradeBand, type StemLabConfig, type StemSubject, type StemSubtype, type StemTool } from './stem-types'
...
export const StemLabEditor: FC<{ lesson: AdminLesson; updateLessonDraft: (lesson: AdminLesson) => void }> = ({ lesson, updateLessonDraft }) => {
  ...
  const chooseToolBySubtype = (subtype: StemSubtype) => {
    const toolDefinition = getStemToolBySubtype(subtype)
    if (toolDefinition) chooseTool(toolDefinition.subject, toolDefinition.tool)
  }
