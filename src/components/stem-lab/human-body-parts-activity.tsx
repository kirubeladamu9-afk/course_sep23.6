...
const bodyParts: BodyPart[] = [
  { id: '1', label: 'Forehead muscle', explanation: 'This muscle helps lift the eyebrows and wrinkle the forehead.', side: 'left', top: '4.5%' },
  { id: '2', label: 'Eye muscle', explanation: 'This ring-shaped muscle helps close and blink the eye.', side: 'left', top: '8%' },
  { id: '3', label: 'Mouth muscle', explanation: 'This muscle helps move the lips for smiling, speaking, and eating.', side: 'left', top: '11.5%' },
  { id: '4', label: 'Neck muscle', explanation: 'This long muscle helps turn and bend the head and neck.', side: 'left', top: '15%' },
  { id: '5', label: 'Upper back muscle', explanation: 'This broad muscle helps support and move the neck and shoulders.', side: 'left', top: '18.5%' },
  { id: '6', label: 'Shoulder muscle', explanation: 'This muscle helps lift the arm away from the body.', side: 'left', top: '22%' },
  { id: '7', label: 'Chest muscle', explanation: 'This muscle helps bring the arm across the front of the body.', side: 'left', top: '25.5%' },
  { id: '8', label: 'Upper arm muscle', explanation: 'This front-of-arm muscle helps bend the elbow.', side: 'right', top: '35%' },
  { id: '9', label: 'Side body muscle', explanation: 'These side muscles help support the trunk and bend the body.', side: 'left', top: '36%' },
  { id: '10', label: 'Abdominal muscle', explanation: 'These front muscles help hold the trunk steady and bend the body.', side: 'right', top: '41%' },
  { id: '11', label: 'Hip muscle', explanation: 'This muscle helps lift and move the leg at the hip.', side: 'left', top: '48%' },
  { id: '12', label: 'Diagonal thigh muscle', explanation: 'This long muscle crosses the thigh and helps bend the hip and knee.', side: 'left', top: '64%' },
  { id: '13', label: 'Inner thigh muscle', explanation: 'This muscle pulls the leg inward toward the body.', side: 'right', top: '64%' },
  { id: '14', label: 'Inner knee muscle', explanation: 'This thigh muscle helps straighten the knee.', side: 'right', top: '70%' },
  { id: '15', label: 'Kneecap', explanation: 'This small bone protects the front of the knee joint.', side: 'left', top: '75%' },
  { id: '16', label: 'Front lower-leg muscle', explanation: 'This muscle helps lift the foot and toes upward.', side: 'left', top: '80%' },
  { id: '17', label: 'Outer calf muscle', explanation: 'This muscle on the outside of the lower leg helps move the foot.', side: 'left', top: '84%' },
  { id: '18', label: 'Calf muscle', explanation: 'This strong muscle helps push the foot down when walking or jumping.', side: 'right', top: '80%' },
  { id: '19', label: 'Side calf muscle', explanation: 'This muscle helps turn and steady the foot and ankle.', side: 'right', top: '84%' },
  { id: '20', label: 'Big-toe lifting muscle', explanation: 'This muscle helps lift the big toe upward.', side: 'left', top: '88%' },
  { id: '21', label: 'Toe-lifting muscles', explanation: 'These muscles help lift and straighten the toes.', side: 'right', top: '88%' },
]
...
const HumanBodyPartsActivity: FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  const renderSlot = (part: BodyPart) => {
    ...
    return <Box key={part.id} component="button" type="button" onClick={() => selectedLabel && placeLabel(part.id, selectedLabel)} onDragOver={(event) => { event.preventDefault(); setHoverSlot(part.id) }} onDragLeave={() => setHoverSlot(null)} onDrop={(event) => handleDrop(event, part.id)} aria-label={`${part.label} pointer${isFilled ? ` labeled ${part.label}` : ', empty'}`} sx={{ position: 'absolute', top: part.top, [part.side]: { xs: '1%', sm: '2%', md: '3%' }, width: { xs: '38%', sm: '20%', md: '16%' }, minHeight: { xs: 34, md: 48 }, p: { xs: .5, md: 1 }, textAlign: 'left', border: 2, borderStyle: 'dashed', borderColor: isHovered ? 'primary.main' : isEmptyAfterCheck ? 'error.main' : isFilled ? 'success.main' : 'rgba(56, 72, 64, .5)', borderRadius: 2, backgroundColor: isHovered ? 'rgba(23, 126, 115, .14)' : isFilled ? 'success.main' : isEmptyAfterCheck ? 'error.light' : 'rgba(255,255,255,.94)', boxShadow: isHovered ? 3 : 1, transform: isHovered ? 'scale(1.03)' : 'none', transition: 'transform .18s ease, border-color .18s ease, background-color .18s ease', color: isFilled ? 'common.white' : 'text.primary', cursor: selectedLabel ? 'pointer' : 'default', zIndex: 2 }}>{isFilled ? <><Typography variant="body2" sx={{ fontWeight: 800, lineHeight: 1.1 }}>{part.label}</Typography><Typography variant="caption" sx={{ display: { xs: 'none', md: 'block' }, color: 'common.white', lineHeight: 1.1 }}>{part.explanation}</Typography></> : <Typography variant="caption" sx={{ fontWeight: 800 }}>Drop label here</Typography>}</Box>
  }
