...
import { PlatformEnginePreview } from './platform-engine-previews'
import { engineForStemTool } from './platform-engine-library'
...
const ToolStep: FC<{ tool: StemTool; onComplete: () => void }> = ({ tool, onComplete }) => {
  ...
  const engineId = engineForStemTool[tool]
  if (tool === 'Drag & Drop') return <DragDropEngine config={dragDropConfig} onComplete={onComplete} />
  if (tool === 'Matching' || tool === 'Shape Matcher') return <MatchingEngine config={matchingConfig} onComplete={onComplete} />
  if (engineId) return <PlatformEnginePreview engineId={engineId} onComplete={onComplete} />
