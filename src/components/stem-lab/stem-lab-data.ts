...
const makeScienceTopics = (subject: Exclude<StemSubject, 'Math'>, titles: string[]) => titles.map((title, index) => {
  ...
  const overview = subject === 'Physics' && title === 'Forces & Motion' ? 'See how forces cause objects to move.' : `Build an animated, interactive understanding of ${title.toLowerCase()} through guided exploration and observable results.`
