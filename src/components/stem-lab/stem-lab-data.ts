...
const makeScienceTopics = (subject: Exclude<StemSubject, 'Math'>, titles: string[]) => titles.map((title, index) => {
  ...
  const overview = subject === 'Physics' && title === 'Forces & Motion' ? 'See how forces cause objects to move.' : subject === 'Physics' && title === 'Friction' ? 'Compare how friction differs across surfaces.' : subject === 'Physics' && title === 'Gravity' ? 'Explore how gravity pulls objects downward.' : `Build an animated, interactive understanding of ${title.toLowerCase()} through guided exploration and observable results.`
