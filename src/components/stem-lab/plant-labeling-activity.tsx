...
const plantParts: PlantPart[] = [
  ...
  { id: 'shoot-system', label: 'Shoot system', explanation: 'The shoot system includes the stem, leaves, flowers, and fruits above the soil.', left: '3%', top: '37%' },
  { id: 'root-system', label: 'Root system', explanation: 'The root system anchors the plant and absorbs water and minerals below the soil.', left: '3%', top: '73%' },
]
...
const PlantLabelingActivity: FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  const renderSlot = (part: PlantPart) => {
    ...
    return <Box key={part.id} component="button" type="button" onClick={() => selectedLabel && placeLabel(part.id, selectedLabel)} onDragOver={(event) => { event.preventDefault(); setHoverSlot(part.id) }} onDragLeave={() => setHoverSlot(null)} onDrop={(event) => handleDrop(event, part.id)} aria-label={`${part.label} pointer${isFilled ? ` labeled ${part.label}` : ', empty'}`} sx={{ position: 'absolute', left: part.left, top: part.top, width: { xs: 120, sm: 150, md: 190 }, minHeight: 52, p: 1, textAlign: 'left', border: 2, borderStyle: 'dashed', borderColor: isHovered ? 'primary.main' : isFilled ? 'success.main' : 'rgba(56, 72, 64, .5)', borderRadius: 2, backgroundColor: isHovered ? 'rgba(23, 126, 115, .14)' : isFilled ? 'success.main' : 'rgba(255,255,255,.92)', boxShadow: isHovered ? 3 : 1, transform: isHovered ? 'scale(1.03)' : 'none', transition: 'transform .18s ease, border-color .18s ease, background-color .18s ease', color: isFilled ? 'common.white' : 'text.primary', cursor: selectedLabel ? 'pointer' : 'default' }}>{isFilled ? <><Stack direction="row" spacing={.5} alignItems="center"><CheckCircleOutlineIcon sx={{ color: 'common.white' }} fontSize="small" /><Typography variant="body2" sx={{ fontWeight: 800 }}>{part.label}</Typography></Stack><Typography variant="caption" sx={{ color: 'common.white' }}>{part.explanation}</Typography></> : <Typography variant="body2" sx={{ fontWeight: 700 }}>Drop label here</Typography>}</Box>
  }
