...
const ChemicalBondingActivity: FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  const lineOrder = bondOrder ?? (order === 3 ? 3 : order === 2 ? 2 : 1)
  ...
  return <Box sx={{ width: '100%', minHeight: 760, color: 'text.primary', backgroundColor: 'background.default', border: '1px solid', borderColor: 'divider', borderRadius: 2, p: { xs: 1, md: 1.5 }, overflow: 'hidden', fontFamily: 'Inter, sans-serif' }}>
    <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} sx={{ minHeight: 730 }}>
      <Paper elevation={0} sx={{ width: { xs: '100%', lg: 250 }, flexShrink: 0, p: 1.25, backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', color: 'inherit' }}>
        ...
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Bond type</Typography><Stack direction="row" spacing={.4} sx={{ mb: 1 }}>{(['Covalent', 'Ionic', 'Metallic'] as BondMode[]).map((item) => <Button key={item} size="small" onClick={() => { setBondMode(item); setTested(false) }} sx={{ minWidth: 0, px: .7, color: bondMode === item ? '#06131b' : 'text.secondary', background: bondMode === item ? '#59e5ba' : 'action.hover', fontSize: 10 }}>{item}</Button>)}</Stack>
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Bond lines</Typography><Stack direction="row" spacing={.4} sx={{ mb: 1 }}>{([{ label: 'Single', value: 1 }, { label: 'Double', value: 2 }, { label: 'Triple', value: 3 }] as const).map((item) => <Button key={item.label} size="small" aria-pressed={bondOrder === item.value} onClick={() => setBondOrder(item.value)} sx={{ minWidth: 0, px: .7, color: lineOrder === item.value ? '#06131b' : 'text.secondary', background: lineOrder === item.value ? '#59e5ba' : 'action.hover', fontSize: 10 }}>{item.label}</Button>)}</Stack>
        ...
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Molecule row</Typography><Stack direction="row" spacing={.35} sx={{ mb: 1, flexWrap: 'wrap' }}>{[['HF', ['H', 'F']], ['HCl', ['H', 'Cl']], ['H₂O', ['O', 'H', 'H']], ['NH₃', ['N', 'H', 'H', 'H']], ['CH₄', ['C', 'H', 'H', 'H', 'H']], ['CF₄', ['C', 'F', 'F', 'F', 'F']], ['CCl₄', ['C', 'Cl', 'Cl', 'Cl', 'Cl']], ['CO₂', ['C', 'O', 'O']], ['H₂', ['H', 'H']], ['N₂', ['N', 'N']], ['O₂', ['O', 'O']], ['F₂', ['F', 'F']], ['Cl₂', ['Cl', 'Cl']], ['NaF', ['Na', 'F']], ['NaCl', ['Na', 'Cl']], ['Na₂O', ['O', 'Na', 'Na']], ['MgCl₂', ['Mg', 'Cl', 'Cl']], ['MgF₂', ['Mg', 'F', 'F']], ['MgO', ['Mg', 'O']], ['Na alloy', ['Na', 'Na']], ['Mg alloy', ['Mg', 'Mg']], ['NaMg alloy', ['Na', 'Mg']]].map(([label, preset]) => <Button key={String(label)} size="small" onClick={() => setPreset(preset as string[])} sx={{ minWidth: 0, color: 'text.secondary', backgroundColor: 'action.hover', fontSize: 10 }}>{label}</Button>)}</Stack>
      ...
      <Box sx={{ minWidth: 0, flex: 1 }}>
        ...
        <Paper elevation={0} sx={{ p: 1, backgroundColor: 'background.default', border: '1px solid #1e4257', color: 'inherit' }}>
          <Stack direction="row" spacing={.5} sx={{ mb: 1, flexWrap: 'wrap' }}>{(['Covalent', 'Ionic', 'Metallic'] as BondMode[]).map((item) => <Button key={item} size="small" onClick={() => setBondMode(item)} sx={{ color: bondMode === item ? '#06131b' : '#8ca8ba', background: bondMode === item ? '#59e5ba' : '#132638', borderRadius: 1 }}>{item}</Button>)}<Divider orientation="vertical" flexItem sx={{ borderColor: '#315269', mx: .25 }} />{([{ label: 'Single', value: 1 }, { label: 'Double', value: 2 }, { label: 'Triple', value: 3 }] as const).map((item) => <Button key={item.label} size="small" aria-pressed={bondOrder === item.value} onClick={() => setBondOrder(item.value)} sx={{ color: lineOrder === item.value ? '#06131b' : '#8ca8ba', background: lineOrder === item.value ? '#59e5ba' : '#132638', borderRadius: 1 }}>{item.label}</Button>)}</Stack>
