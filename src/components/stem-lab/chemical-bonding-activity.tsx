...
const EnergyChart: FC<{ history: EngineSnapshot[] }> = ({ history }) => {
  ...
  return <Box component="svg" viewBox="0 0 310 110" sx={{ width: '100%', height: 105, background: '#f7fbfb', borderRadius: 1.5, border: '1px solid', borderColor: 'divider' }} aria-label="Live kinetic potential and total energy chart"><path d="M20 12V92H295" fill="none" stroke="#55718c" strokeWidth="1" />{values.length > 1 && <><polyline points={path('kinetic')} fill="none" stroke="#ffc76b" strokeWidth="1.8" /><polyline points={path('potential')} fill="none" stroke="#63e9c2" strokeWidth="1.8" /><polyline points={path('total')} fill="none" stroke="#a9a5ff" strokeWidth="2" /></>}<text x="23" y="105" fill="#7190a9" fontSize="9">time (fs)</text><text x="7" y="15" fill="#7190a9" fontSize="9">kJ/mol</text><text x="198" y="18" fill="#ffc76b" fontSize="9">K</text><text x="218" y="18" fill="#63e9c2" fontSize="9">V</text><text x="238" y="18" fill="#a9a5ff" fontSize="9">E</text></Box>
}
...
const PotentialCurve: FC<{ distancePm: number; equilibriumPm: number; energyKjMol: number }> = ({ distancePm, equilibriumPm, energyKjMol }) => {
  ...
  return <Box component="svg" viewBox="0 0 310 110" sx={{ width: '100%', height: 105, background: '#f7fbfb', borderRadius: 1.5, border: '1px solid', borderColor: 'divider' }} aria-label="Morse potential energy versus distance"><path d="M20 12V92H295" fill="none" stroke="#55718c" strokeWidth="1" /><polyline points={points} fill="none" stroke="#63e9c2" strokeWidth="2.5" /><line x1={markerX} x2={markerX} y1="10" y2="92" stroke="#ffc76b" strokeDasharray="3 3" /><circle cx={markerX} cy={50} r="4" fill="#ffc76b" /><text x="23" y="105" fill="#7190a9" fontSize="9">short range</text><text x="240" y="105" fill="#7190a9" fontSize="9">distance →</text><text x="6" y="15" fill="#7190a9" fontSize="9">V(r)</text></Box>
}
...
const ChemicalBondingActivity: FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  const [bondOrder, setBondOrder] = useState<1 | 2 | 3 | null>(null)
  ...
  const isCarbonMonoxide = symbols.length === 2 && symbols.includes('C') && symbols.includes('O')
  const lineOrder = isCarbonMonoxide ? 3 : bondOrder ?? (order === 3 ? 3 : order === 2 ? 2 : 1)
  ...
  const selectElement = (symbol: string) => { setBondOrder(null); setSlots((current) => current.length >= 5 ? [...current.slice(1), symbol] : [...current, symbol]) }
  const setPreset = (preset: string[], lineOrder: 1 | 2 | 3 | null = null) => { setSlots(preset); setBondOrder(lineOrder); setTested(true) }
  const selectBondMode = (mode: BondMode) => { setBondMode(mode); setBondOrder(null); setTested(false); setSlots(mode === 'Ionic' ? ['Na', 'Cl'] : mode === 'Metallic' ? ['Na', 'Na'] : ['H', 'H']) }
  ...
  const reset = () => { setSlots([]); setBondOrder(null); setTested(false); setPlaying(true); setHistory([]) }
  ...
  const ionicPresetLabels = ['NaF', 'NaCl', 'Na₂O', 'MgCl₂', 'MgF₂', 'MgO']
  const metallicPresetLabels = ['Na alloy', 'Mg alloy', 'NaMg alloy']

  return <Box sx={{ width: '100%', minHeight: 760, color: 'text.primary', backgroundColor: 'background.default', border: '1px solid', borderColor: 'divider', borderRadius: 2, p: { xs: 1, md: 1.5 }, overflow: 'hidden', fontFamily: 'Inter, sans-serif' }}>
    <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} sx={{ minHeight: 730 }}>
      <Paper elevation={0} sx={{ width: { xs: '100%', lg: 250 }, flexShrink: 0, p: 1.25, backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', color: 'inherit' }}>
        <Typography variant="subtitle2" sx={{ color: '#64e7c1', fontWeight: 800 }}>Build a Compound</Typography><Typography variant="caption" sx={{ display: 'block', color: 'text.secondary', mb: 1 }}>Select atoms, test a bond, then inspect live forces and electron sharing.</Typography>
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Bond type</Typography><Stack direction="row" spacing={.4} sx={{ mb: 1 }}>{(['Covalent', 'Ionic', 'Metallic'] as BondMode[]).map((item) => <Button key={item} size="small" onClick={() => selectBondMode(item)} sx={{ minWidth: 0, px: .7, color: bondMode === item ? '#06131b' : 'text.secondary', background: bondMode === item ? '#59e5ba' : 'action.hover', fontSize: 10 }}>{item}</Button>)}</Stack>
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Element grid</Typography><Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: .45, mb: 1 }}>{CHEMICAL_ELEMENTS.map((element) => <Button key={element.symbol} onClick={() => selectElement(element.symbol)} sx={{ minWidth: 0, p: .35, display: 'flex', flexDirection: 'column', border: '1px solid #26455c', color: '#000', background: `${element.color}33`, '&:hover': { background: `${element.color}88` } }}><Typography sx={{ fontWeight: 800, fontSize: 12 }}>{element.symbol}</Typography><Typography sx={{ fontSize: 8, color: '#000' }}>{element.atomicNumber}</Typography></Button>)}</Box>
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Molecule row</Typography><Stack direction="row" spacing={.35} sx={{ mb: 1, flexWrap: 'wrap' }}>{[['HF', ['H', 'F']], ['HCl', ['H', 'Cl']], ['H₂O', ['O', 'H', 'H']], ['NH₃', ['N', 'H', 'H', 'H']], ['CH₄', ['C', 'H', 'H', 'H', 'H']], ['CF₄', ['C', 'F', 'F', 'F', 'F']], ['CCl₄', ['C', 'Cl', 'Cl', 'Cl', 'Cl']], ['CO₂', ['C', 'O', 'O']], ['CO', ['C', 'O']], ['H₂', ['H', 'H']], ['N₂', ['N', 'N']], ['O₂', ['O', 'O']], ['F₂', ['F', 'F']], ['Cl₂', ['Cl', 'Cl']], ['NaF', ['Na', 'F']], ['NaCl', ['Na', 'Cl']], ['Na₂O', ['O', 'Na', 'Na']], ['MgCl₂', ['Mg', 'Cl', 'Cl']], ['MgF₂', ['Mg', 'F', 'F']], ['MgO', ['Mg', 'O']], ['Na alloy', ['Na', 'Na']], ['Mg alloy', ['Mg', 'Mg']], ['NaMg alloy', ['Na', 'Mg']]].filter(([label]) => bondMode === 'Ionic' ? ionicPresetLabels.includes(String(label)) : bondMode === 'Metallic' ? metallicPresetLabels.includes(String(label)) : !ionicPresetLabels.includes(String(label)) && !metallicPresetLabels.includes(String(label))).map(([label, preset]) => <Button key={String(label)} size="small" onClick={() => setPreset(preset as string[], label === 'CO' ? 3 : null)} sx={{ minWidth: 0, color: 'text.secondary', backgroundColor: 'action.hover', fontSize: 10 }}>{label}</Button>)}</Stack>
        <Divider sx={{ borderColor: '#1e3a50', mb: 1 }} /><Typography variant="overline" sx={{ color: 'text.secondary' }}>Selected atoms</Typography><Stack direction="row" spacing={.5} sx={{ mb: 1, flexWrap: 'wrap' }}>{[0, 1, 2, 3, 4].map((index) => <Box key={index} sx={{ flex: '1 0 28px', p: .65, minHeight: 34, border: '1px dashed #41617a', borderRadius: 1, textAlign: 'center', color: slots[index] ? '#64e7c1' : '#678296' }}>{slots[index] ?? 'empty'}</Box>)}</Stack>
        ...
        <Stack direction="row" spacing={.5} sx={{ mt: 1 }}><Button size="small" onClick={reset} startIcon={<ReplayIcon />} sx={{ color: 'text.secondary', flex: 1 }}>Reset</Button><Tooltip title={muted ? 'Unmute sound' : 'Mute sound'}><IconButton size="small" onClick={() => setMuted(!muted)} sx={{ color: 'text.secondary' }}>{muted ? <VolumeOffIcon fontSize="small" /> : <VolumeUpIcon fontSize="small" />}</IconButton></Tooltip></Stack>
        <Box sx={{ mt: 2, p: .8, borderRadius: 1, backgroundColor: 'action.hover', color: 'text.secondary', fontSize: 10 }}>Bohr shells are a teaching model. Forces and coordinates come from the TypeScript worker, not scripted animation.</Box>
      </Paper>
      <Box sx={{ minWidth: 0, flex: 1 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={1} sx={{ mb: 1 }}><Box><Typography variant="h6" sx={{ color: '#eaf8fb', fontWeight: 800 }}>Chemical Bonding Simulator</Typography><Typography variant="caption" sx={{ color: 'text.secondary' }}>Orbit the real molecule, stretch a bond, and inspect energy in every frame.</Typography></Box><Chip label={stable ? 'Live physics' : 'Bond dissociated'} size="small" sx={{ color: '#06201b', background: stable ? '#59e5ba' : '#ffc76b' }} /></Stack>
        <Paper elevation={0} sx={{ p: 1, backgroundColor: 'background.default', border: '1px solid #1e4257', color: 'inherit' }}>
          <Stack direction="row" spacing={.5} sx={{ mb: 1 }}>{(['Covalent', 'Ionic', 'Metallic'] as BondMode[]).map((item) => <Button key={item} size="small" onClick={() => selectBondMode(item)} sx={{ color: bondMode === item ? '#06131b' : '#8ca8ba', background: bondMode === item ? '#59e5ba' : '#132638', borderRadius: 1 }}>{item}</Button>)}</Stack>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }} sx={{ mt: 1 }}><Typography variant="caption" sx={{ minWidth: 75, color: 'text.secondary' }}>Slow motion</Typography><Slider size="small" min={.15} max={1.5} step={.05} value={slowMotion} onChange={(_, value) => setSlowMotion(value as number)} sx={{ maxWidth: 150, color: '#59e5ba' }} aria-label="Slow motion" /><Typography variant="caption" sx={{ color: 'text.secondary' }}>{slowMotion.toFixed(2)}×</Typography><Typography variant="caption" sx={{ minWidth: 60, color: 'text.secondary' }}>Temperature</Typography><Slider size="small" min={0} max={1200} step={10} value={temperature} onChange={(_, value) => setTemperature(value as number)} sx={{ maxWidth: 150, color: '#59e5ba' }} aria-label="Temperature in kelvin" /><Typography variant="caption" sx={{ color: 'text.secondary' }}>{temperature} K</Typography><Typography variant="caption" sx={{ color: 'text.secondary' }}>Orbit</Typography><Slider size="small" min={-180} max={180} value={rotation} onChange={(_, value) => setRotation(value as number)} sx={{ maxWidth: 130, color: '#59e5ba' }} aria-label="Rotate molecule" /></Stack>
        </Paper>
        {workerError && <Typography variant="caption" sx={{ display: 'block', mt: 1, color: '#ffadbd' }}>{workerError}</Typography>}
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1} sx={{ mt: 1 }}><Paper elevation={0} sx={{ flex: 1, p: 1, backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', color: 'inherit' }}><Typography variant="subtitle2" sx={{ color: '#64e7c1' }}>{formula} — {kind} model</Typography><Typography variant="caption" sx={{ display: 'block', color: '#a6bac6' }}>Bond length: <b>{Math.round(distancePm)} pm</b> · Bond energy: <b>{reference.energyKjMol} kJ/mol</b> · order <b>{order === 3 ? 'triple' : order === 2 ? 'double' : 'single'}</b></Typography><Typography variant="caption" sx={{ display: 'block', color: 'text.secondary' }}>Electrons: {shellLegend}</Typography>{showValence && <Typography variant="caption" sx={{ display: 'block', color: '#6ff5d4', mt: .4 }}>{valenceText}</Typography>}<Typography variant="caption" sx={{ display: 'block', color: 'text.secondary', mt: .5 }}>{first.symbol === second.symbol ? 'Nonpolar bond' : `${first.symbol}–${second.symbol}: ΔEN ${polarity.toFixed(2)} · ${polarity > 0.4 ? `${first.electronegativity > second.electronegativity ? first.symbol : second.symbol} δ−` : 'symmetric sharing'}`}</Typography>{snapshot && <Typography variant="caption" sx={{ display: 'block', color: 'text.secondary', mt: .5 }}>K {snapshot.energy.kinetic.toFixed(2)} · V {snapshot.energy.potential.toFixed(2)} · E {snapshot.energy.total.toFixed(2)} kJ/mol · {snapshot.energy.temperatureK.toFixed(0)} K</Typography>}</Paper><Paper elevation={0} sx={{ flex: 1, p: 1, backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', color: 'inherit' }}><Typography variant="subtitle2" sx={{ color: '#64e7c1' }}>Energy / distance</Typography><PotentialCurve distancePm={distancePm} equilibriumPm={reference.lengthPm} energyKjMol={reference.energyKjMol} /><Typography variant="caption" sx={{ color: 'text.secondary' }}>Drag the molecule horizontally to pull a two-atom bond until its Morse well rises.</Typography></Paper></Stack>
        <Paper elevation={0} sx={{ mt: 1, p: 1, backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', color: 'inherit' }}><Typography variant="subtitle2" sx={{ color: '#64e7c1' }}>Live energy chart</Typography><EnergyChart history={history} /><Typography variant="caption" sx={{ color: 'text.secondary' }}>Velocity Verlet · 0.5 fs timestep · {thermostat ? 'Berendsen thermostat enabled' : 'thermostat off: energy should remain nearly conserved'}</Typography></Paper>
      </Box>
    </Stack>
  </Box>
}
