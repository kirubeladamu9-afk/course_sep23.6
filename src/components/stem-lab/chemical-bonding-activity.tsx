...
type LearningMode = 'Explore' | 'Quiz'
...
const ChemicalBondingActivity: FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  const validPair = bondMode === 'Metallic' || (first.symbol !== second.symbol || first.symbol === 'H')
  ...
  const reset = () => { setSlots(['C', 'H']); setTested(false); setPlaying(true); setHistory([]) }
  const exportModel = () => {
    const model = { engine: 'chemical-bonding-worker', mode: bondMode, formula, atoms: symbols, bondLengthPm: Math.round(distancePm), bondEnergyKjMol: reference.energyKjMol, bondOrder: order, energy: snapshot?.energy, sources: ['Universal Force Field (Rappe et al., JACS 1992)', 'Blue Obelisk Data Repository', 'NIST CCCBDB'] }
    const url = URL.createObjectURL(new Blob([JSON.stringify(model, null, 2)], { type: 'application/json' }))
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'chemical-bonding-model.json'; anchor.click(); URL.revokeObjectURL(url)
  }
  ...
  return <Box sx={{ width: '100%', minHeight: 760, color: '#d9eef7', background: 'linear-gradient(145deg,#050b16,#0b1625 55%,#09111d)', border: '1px solid #1e4257', borderRadius: 2, p: { xs: 1, md: 1.5 }, overflow: 'hidden', fontFamily: 'Inter, sans-serif' }}>
    <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} sx={{ minHeight: 730 }}>
      <Paper elevation={0} sx={{ width: { xs: '100%', lg: 250 }, flexShrink: 0, p: 1.25, background: '#0b1524', border: '1px solid #1c3851', color: 'inherit' }}>
        ...
        <Typography variant="overline" sx={{ color: '#7795aa' }}>Learning mode</Typography><Stack direction="row" spacing={.4} sx={{ mb: 1 }}>{(['Explore', 'Quiz'] as LearningMode[]).map((item) => <Button key={item} size="small" onClick={() => setLearningMode(item)} sx={{ minWidth: 0, px: 1, color: learningMode === item ? '#06131b' : '#9fb6c6', background: learningMode === item ? '#59e5ba' : '#15283a', fontSize: 10 }}>{item}</Button>)}</Stack>
        ...
        <Divider sx={{ borderColor: '#1e3a50', mb: 1 }} /><Typography variant="overline" sx={{ color: '#7795aa' }}>Selected atoms</Typography><Stack direction="row" spacing={.5} sx={{ mb: 1, flexWrap: 'wrap' }}>{slots.map((symbol, index) => <Box key={`${symbol}-${index}`} sx={{ flex: '1 0 28px', p: .65, minHeight: 34, border: '1px dashed #41617a', borderRadius: 1, textAlign: 'center', color: '#64e7c1' }}>{symbol}</Box>)}</Stack>
      ...
      <Box sx={{ minWidth: 0, flex: 1 }}>
        ...
        {learningMode === 'Quiz' && <Paper elevation={0} sx={{ mt: 1, p: 1, background: '#102336', border: '1px solid #31506a', color: 'inherit' }}><Typography variant="subtitle2" sx={{ color: '#64e7c1' }}>Quiz mode</Typography><Typography variant="caption">Predict the bond: ΔEN = {polarity.toFixed(2)}. The engine classifies this pair as <b>{kind}</b>; inspect the potential curve and then press Test Bond.</Typography></Paper>}
