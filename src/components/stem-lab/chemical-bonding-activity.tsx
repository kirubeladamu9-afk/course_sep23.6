  ...
  return <Box sx={{ width: '100%', minHeight: 760, color: 'text.primary', backgroundColor: 'background.default', border: '1px solid', borderColor: 'divider', borderRadius: 2, p: { xs: 1, md: 1.5 }, overflow: 'hidden', fontFamily: 'Inter, sans-serif' }}>
    <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} sx={{ minHeight: 730 }}>
      <Paper elevation={0} sx={{ width: { xs: '100%', lg: 250 }, flexShrink: 0, p: 1.25, backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', color: 'inherit' }}>
        ...
        <Typography variant="overline" sx={{ color: 'text.secondary' }}>Bond lines</Typography><Stack direction="row" spacing={.4} sx={{ mb: 1 }}>{([{ label: 'Single', value: 1 }, { label: 'Double', value: 2 }, { label: 'Triple', value: 3 }] as const).map((item) => <Button key={item.label} size="small" aria-pressed={bondOrder === item.value} onClick={() => setBondOrder(item.value)} sx={{ minWidth: 0, px: .7, color: bondOrder === item.value ? '#06131b' : 'text.secondary', background: bondOrder === item.value ? '#59e5ba' : 'action.hover', fontSize: 10 }}>{item.label}</Button>)}</Stack>
