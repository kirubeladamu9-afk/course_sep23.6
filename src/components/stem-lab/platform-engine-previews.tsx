...
const ArrayBuilderActivity: FC<{ topic: MathTopic; onComplete?: () => void }> = ({ topic, onComplete }) => {
  ...
  return <EngineFrame name={topic.activity} description={topic.activityDescription}><Stack direction="row" spacing={1} justifyContent="center"><Button variant={mode === 'groups' ? 'contained' : 'outlined'} onClick={() => { setMode('groups'); reset() }}>Equal groups</Button><Button variant={mode === 'sharing' ? 'contained' : 'outlined'} onClick={() => { setMode('sharing'); reset() }}>Fair sharing</Button></Stack>{mode === 'groups' ? <><Typography variant="h6" sx={{ textAlign: 'center' }}>Build {groupCount} groups of {groupSize}</Typography><Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>Tap critter spaces to place or remove one.</Typography><Stack spacing={1}>{groupRows.map((row, rowIndex) => <Stack key={rowIndex} direction="row" spacing={.5} alignItems="center" sx={{ p: .75, borderRadius: 1.5, backgroundColor: row.length === groupSize ? 'success.light' : row.length ? 'warning.light' : 'background.default' }}><Typography sx={{ width: 58, fontWeight: 700 }}>Group {rowIndex + 1}</Typography>{Array.from({ length: groupSize }, (_, cellIndex) => <Button key={cellIndex} aria-label={`Group ${rowIndex + 1} critter ${cellIndex + 1}`} onClick={() => toggleCritter(rowIndex, cellIndex)} sx={{ minWidth: 36, height: 36, p: 0, borderRadius: '50%', backgroundColor: cellIndex < row.length ? 'primary.main' : 'action.hover', color: cellIndex < row.length ? 'common.white' : 'text.secondary' }}>{cellIndex < row.length ? '●' : '+'}</Button>)}<Typography variant="caption">{row.length}</Typography></Stack>)}</Stack><Typography variant="h6" sx={{ textAlign: 'center', fontFamily: 'monospace' }}>{Array.from({ length: groupCount }, (_, index) => groupRows[index]?.length ?? 0).join(' + ')} = {groupTotal}</Typography><Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>{groupSize} × {groupCount} = {groupCount * groupSize}</Typography></> : <><Typography variant="h6" sx={{ textAlign: 'center' }}>Share {shareTotal} critters fairly among {basketCount} baskets</Typography><Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>Critters left in pile: {shareTotal - baskets.reduce((sum, count) => sum + count, 0)}</Typography><Stack direction="row" spacing={1} justifyContent="center" flexWrap="wrap"><Chip label={`Pile: ${shareTotal - baskets.reduce((sum, count) => sum + count, 0)} critters`} color="warning" />{baskets.map((count, index) => <Paper key={index} elevation={0} sx={{ p: 1, minWidth: 82, textAlign: 'center', border: 2, borderColor: count === shareTotal / basketCount ? 'success.main' : 'divider', backgroundColor: count === shareTotal / basketCount ? 'success.light' : 'background.default' }}><Typography variant="caption">Basket {index + 1}</Typography><Typography variant="h6">{count}</Typography><Stack direction="row" justifyContent="center"><Button size="small" onClick={() => removeFromBasket(index)} aria-label={`Remove from basket ${index + 1}`}>−</Button><Button size="small" onClick={() => addToBasket(index)} aria-label={`Add to basket ${index + 1}`}>+</Button></Stack></Paper>)}</Stack><Typography variant="body2" sx={{ textAlign: 'center', fontFamily: 'monospace' }}>{baskets.join(' + ')} = {baskets.reduce((sum, count) => sum + count, 0)} · {shareTotal} ÷ {basketCount} = {shareTotal / basketCount}</Typography></>}
