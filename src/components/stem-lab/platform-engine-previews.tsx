...
const MathActivityEngine: FC<{ topic: MathTopic; onComplete?: () => void }> = ({ topic, onComplete }) => {
  ...
  if (topic.title === 'Sequences & Series') return frame(<><Stack direction="row" spacing={1} justifyContent="center">{Array.from({ length: 5 }, (_, index) => <Button key={index} variant={index < value % 5 ? 'contained' : 'outlined'} onClick={() => update(Math.min(topic.target, value + 1))}>●</Button>)}</Stack><Typography>Build the next term: {value}</Typography><Slider min={0} max={topic.target} value={value} onChange={(_, next) => update(Array.isArray(next) ? next[0] : next)} aria-label="Sequence builder" /></>)
