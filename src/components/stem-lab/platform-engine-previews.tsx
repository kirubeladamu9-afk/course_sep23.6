...
const MathActivityEngine: FC<{ topic: MathTopic; onComplete?: () => void }> = ({ topic, onComplete }) => {
  ...
  if (topic.title === 'Measurement') return frame(<><Box sx={{ position: 'relative', height: 100, borderBottom: 4, borderColor: 'divider' }}><Box sx={{ position: 'absolute', left: `${Math.min(value * 8, 88)}%`, bottom: -12, width: 12, height: 30, backgroundColor: 'secondary.main' }} /></Box><Typography>Drag the ruler marker until the object measures {topic.target} units.</Typography><Slider min={0} max={12} value={value} onChange={(_, next) => update(Array.isArray(next) ? next[0] : next)} aria-label="Ruler measurement" /></>)
